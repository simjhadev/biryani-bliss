...
const OrdersPage = () =>{
    ...
    const { isPending, data, isError, error } = useQuery({
        queryKey: ['orders'],
        queryFn: () => fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/orders`).then(
            (res) => res.json(),
        ),
      });
        ...
        <div className="p-4 lg:px-20 xl:px-40">
            <table className="w-full">
                ...
                <tbody>
                    ...
                    <tr className={`${item.status.toLowerCase() !== "delivered" && "bg-red-50"}`} key={item.id}>
                        <td className="hidden md:block py-6 px-1">{item.id.toUpperCase()}</td>
                        ...
                        {session?.user.isAdmin ? (
                            ...
                            <button className="bg-red-400 p-3 rounded-full">
                                <Image src="/edit.png" alt="Status edit button" width={20} height={20} />
                            </button>
                                </form>
                            </td>
                        ) : (
